...
,t_patient.patient_contact_house
 || ' ม. ' || t_patient.patient_contact_moo
 || ' ถ.' || t_patient.patient_contact_road
 || ' ต.' || tambon_ct.address_description
 || ' อ.' || amphur_ct.address_description
 || ' จ.' || changwat_ct.address_description as telleraddress
...
from t_visit left join t_visit_vital_sign on t_visit_vital_sign.t_visit_id = t_visit.t_visit_id
              ...
              left join f_patient_marriage_status on t_patient.f_patient_marriage_status_id = f_patient_marriage_status.f_patient_marriage_status_id
                ...
                left join (select * from f_address) as changwat_ct on changwat_ct.f_address_id = t_patient.patient_contact_changwat
                left join (select * from f_address) as amphur_ct on amphur_ct.f_address_id = t_patient.patient_contact_amphur
                left join (select * from f_address) as tambon_ct on tambon_ct.f_address_id = t_patient.patient_contact_tambon
...
and   t_visit.t_visit_id = '255113545179678911'--$P{visit_id}
...
,t_patient.patient_contact_house
,t_patient.patient_contact_moo
,t_patient.patient_contact_road
